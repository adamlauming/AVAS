%% method: increase random noise from -1~1(the mean is 0)
clear all;
close all;
clc;
...
for index = 1:length(speedlist)
    ...
    if speed_o <= 20
        ...
        elseif index >= 2 && index < length(speedlist)
            speed_before = speed;
            s_out_before = s_out;
            speed = speedlist(index)+round(rand(1))
            ...
            if speed == speed_before || speed_before > 20
                ...
                s_out(1:samples_min/4) = s_out(1:samples_min/4) + s_out_before((samples_min-samples_min/4+1):samples_min); 
            end
    ...
    audiowrite('output2.wav', s_output, Fs); 
end
